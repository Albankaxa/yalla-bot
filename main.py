...
async def wait_photo(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    user_state[user_id] = "photo"
    await callback_query.message.answer("📷 Пожалуйста, пришлите фото:")
...
async def skip_photo(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        await callback_query.message.answer(f"❌ Ошибка при отправке: {str(e)}")
        return
...
async def handle_moderation(callback_query: types.CallbackQuery):
    ...
    if action == "approve":
        try:
            ...
            await callback_query.answer(f"❌ Ошибка при публикации: {str(e)}")
    else:
